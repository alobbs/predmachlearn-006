...
#### Cleaning variables
Clearly there are manyNA or useless or empty values of variables for the prediction. The prediction should only be computed on the accelerometers values of belt, forearm, arm and dumbell. So, the non-accelerometer measures are discard with the useless variables.
...
Let's get rid of the variables with either zero value or a value near zero. Those entrues don't contain relevan information for the predictions:
...
At this stage, set is divided in two parts. The first one for training, and the other for validation
